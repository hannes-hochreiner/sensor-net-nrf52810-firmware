...
impl Power {
    ...
    pub fn set_mode(&mut self, mode: Mode) {
        match mode {
            Mode::ConstantLatency => {
                self.power
                    .tasks_constlat
                    .write(|w| w.tasks_constlat().set_bit());
            }
            Mode::LowPower => {
                self.power
                    .tasks_lowpwr
                    .write(|w| w.tasks_lowpwr().set_bit());
            }
        }
    }
    ...
    pub fn clear_events(&mut self) {
        self.power
            .events_sleepenter
            .write(|w| w.events_sleepenter().clear_bit());
        self.power
            .events_sleepexit
            .write(|w| w.events_sleepexit().clear_bit());
    }

    pub fn is_event_set(&self, event: Event) -> bool {
        match event {
            Event::SleepEnter => self
                .power
                .events_sleepenter
                .read()
                .events_sleepenter()
                .is_generated(),
            Event::SleepExit => self
                .power
                .events_sleepexit
                .read()
                .events_sleepexit()
